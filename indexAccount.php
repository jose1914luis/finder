<?php
	...
	else if(@$_GET["mnu"]=="prospectos_rsl") 		$paginaCargue = "v_prospectos.rsl.php";		
	...
	else if(@$_GET["mnu"]=="noticias") 				$paginaCargue = "v_noticias.php";
		...
		if(@$_POST["txtDocumento"]!="") {
			...
			if($docUsuario == $_POST["txtDocumento"]) {
				...
				if(@$_GET["credits"]!="1")
					...
					$IncludeVista = 0;
					echo "
						<script>alert('Informaci\u00F3n almacenada correctamente. Contin\u00FAa con tu compra de cr\u00E9ditos'); document.location.href='?mnu=creditos';</script>
					";
				} 					
			} else
	...
	else if(@$_GET["mnu"]=="change_passwd")			$paginaCargue = "v_passwd.php";
	else if(@$_GET["mnu"]=="logout") 				$paginaCargue = "v_exit.php";
		...
		if(@$_POST["coordenadasPry"]!="") {
			...
			if($area <= $_SESSION['rango_superior']) {
				...
				if(@$_POST["tipoOperaPry"]=="crear prospecto") {	
					...
					if($resultado == "OK")	$accionPry = "document.location.href='?fnd=prospectos&crd=prospecto&placa=$placa&clasificacion=PROSPECTO'";
					else					$accionPry = "alert('Error en la generaci\u00F3n del Prospecto $placa'); window.close();";			
					echo "<script>$accionPry</script>";	
				} 
				else if(@$_POST["tipoOperaPry"]=="superponer") {	
					$placa 					= $prp->crearProspecto();
					$resultado 				= $prp->insertAll($_POST, $_SESSION["idEmpresa"], $_SESSION["id_usuario"], $placa);		
					if($resultado == "OK")	$accionPry = "document.location.href='?fnd=prospectos&crd=prospecto&placa=$placa&clasificacion=ESTUDIO_TECNICO_PROSPECTO&credits=1'";
					else					$accionPry = "aler('Error en la generaci\u00F3n del Reporte de Area Libre de $placa'); window.close();";			
					echo "<script>$accionPry</script>";	
				} 
				else
